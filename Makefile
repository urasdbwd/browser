...
build-pi: download-v8
	@printf "\033[36mGenerating V8 snapshot for the Raspberry Pi build...\033[0m\n"
	@CARGO_BUILD_JOBS=$(PI_JOBS) $(ZIG) build -j$(PI_JOBS) -Dprebuilt_v8_path=$(V8_CACHE) -Doptimize=ReleaseSafe snapshot_creator -- src/snapshot.bin || (printf "\033[33mSnapshot build ERROR\033[0m\n"; exit 1;)
	@printf "\033[36mBuilding Raspberry Pi profile ($(PI_OPTIMIZE), $(PI_JOBS) jobs)...\033[0m\n"
	@CARGO_BUILD_JOBS=$(PI_JOBS) $(ZIG) build -j$(PI_JOBS) -Dprebuilt_v8_path=$(V8_CACHE) -Doptimize=$(PI_OPTIMIZE) -Dlow_resource_default=true -Dsnapshot_path=../../snapshot.bin || (printf "\033[33mPi build ERROR\033[0m\n"; exit 1;)
