...
// console's members are static namespace operations (no brand check), so they
// reach their state through here rather than through a `self` receiver.
pub fn console(self: *const Execution) *Console {
    return switch (self.js.global) {
        inline else => |g| g.console(),
    };
}
